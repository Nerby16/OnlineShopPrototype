...
export default function AdminOrderDetail({ orderId }: { orderId: number }) {
  ...
  if (loading) return <main className="order-detail-loading admin"><a className="brand" href="/">NEXO ANIMAL</a><p>Abriendo la información del pedido…</p></main>;
  if (!user || !order) return <main className="order-detail-error"><a className="brand" href="/">NEXO ANIMAL</a><div><p className="eyebrow">Acceso administrativo</p><h1>No podemos abrir<br /><em>este pedido.</em></h1><p>{error}</p><a className="primary-link" href="/admin">Ir a administración <span>→</span></a></div></main>;

  return (
    <main className="order-detail-page admin-order-detail-page">
      <header className="account-header"><a className="brand" href="/">NEXO ANIMAL</a><nav><a href="/admin#pedidos">← Todos los pedidos</a><span>Sesión de {user.name}</span></nav></header>
