...
export default function AdminCustomerDetail({ customerId }: { customerId: number }) {
  ...
  if (loading) return <main className="order-detail-loading admin"><a className="brand" href="/">NEXO ANIMAL</a><p>Preparando la ficha del cliente…</p></main>;
  if (!user || !customer) return <main className="order-detail-error"><a className="brand" href="/">NEXO ANIMAL</a><div><p className="eyebrow">Acceso administrativo</p><h1>No podemos abrir<br /><em>este cliente.</em></h1><p>{error}</p><a className="primary-link" href="/admin#clientes">Volver a clientes <span>→</span></a></div></main>;
  ...
  return (
    <main className="order-detail-page admin-customer-detail-page">
      <header className="account-header"><a className="brand" href="/">NEXO ANIMAL</a><nav><a href="/admin#clientes">← Todos los clientes</a><span>Sesión de {user.name}</span></nav></header>
