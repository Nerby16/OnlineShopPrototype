...
export default function OrderTimeline({ order }: { order: OrderDetail }) {
  ...
  const steps = [
    ...
    { label: "Preparando", detail: "En nuestro estudio", date: order.paidAt },
  ...
  return (
    <div>
      {order.status === "cancelled" && (
        <div className="order-cancelled-note"><strong>Pedido cancelado</strong><span>{formatDate(order.cancelledAt)}</span><p>Las piezas se devolvieron automáticamente al stock disponible.</p></div>
      )}
