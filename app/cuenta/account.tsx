...
export default function AccountArea() {
  ...
  if (checking) return <main className="account-loading"><a className="brand" href="/">LÚMINA</a><p>Preparando tu espacio personal…</p></main>;

  if (!user) {
    return (
      <main className="account-auth">
        <header className="account-header"><a className="brand" href="/">LÚMINA</a><a href="/">← Volver a la tienda</a></header>
        <section className="account-auth-layout">
          <div className="account-auth-copy"><p className="eyebrow">Tu espacio Lúmina</p><h1>Todo lo que eliges,<br /><em>en un solo lugar.</em></h1><p>Consulta pedidos, guarda favoritos y reutiliza tus direcciones en cada compra.</p><ul><li>Historial y cancelación de pedidos pendientes</li><li>Favoritos y direcciones guardadas</li><li>Sesión privada y contraseña cifrada</li></ul></div>
          <form className="account-auth-form" onSubmit={authenticate}>
            ...
            <div className="form-grid">{mode === "register" && <label className="field field-wide"><span>Nombre completo</span><input name="name" autoComplete="name" required /></label>}<label className="field field-wide"><span>Correo electrónico</span><input name="email" type="email" autoComplete="email" required /></label><label className="field field-wide"><span>Contraseña</span><input name="password" type="password" minLength={8} maxLength={128} autoComplete={mode === "login" ? "current-password" : "new-password"} required /></label>{mode === "register" && <label className="field field-wide"><span>Repite la contraseña</span><input name="confirmation" type="password" minLength={8} maxLength={128} autoComplete="new-password" required /></label>}</div>
  ...
  return (
    <main className="account-page">
      <header className="account-header"><a className="brand" href="/">LÚMINA</a><nav><a href="/">Tienda</a>{user.role === "admin" && <a href="/admin">Administración</a>}<button type="button" onClick={logout}>Cerrar sesión</button></nav></header>
      <section className="account-hero"><div><p className="eyebrow">Mi cuenta</p><h1>Hola, <em>{user.name.split(" ")[0]}.</em></h1><p>{user.email}</p></div><span>Cliente Lúmina · #{String(user.id).padStart(4, "0")}</span></section>
      <section className="account-dashboard">
        ...
        <section className="account-subsection" id="seguridad"><div className="account-orders-heading"><div><p className="eyebrow">Seguridad</p><h2>Cambiar contraseña</h2></div></div><form className="account-settings-form security" onSubmit={changePassword}><div className="form-grid"><label className="field"><span>Contraseña actual</span><input name="currentPassword" type="password" autoComplete="current-password" required /></label><label className="field"><span>Nueva contraseña</span><input name="newPassword" type="password" minLength={8} maxLength={128} autoComplete="new-password" required /></label><label className="field"><span>Repite la nueva contraseña</span><input name="confirmation" type="password" minLength={8} maxLength={128} autoComplete="new-password" required /></label></div><button className="checkout-submit" type="submit" disabled={loading}>Actualizar contraseña <span>→</span></button><small>Se cerrarán las demás sesiones abiertas y esta se renovará automáticamente.</small></form></section>
      </section>
      {cancellationOrder && (
        <div className="account-confirmation-overlay" role="dialog" aria-modal="true" aria-labelledby="cancel-order-title">
          ...
          <section className="account-confirmation-panel">
            ...
            <p>Esta acción marcará el pedido como cancelado y devolverá sus {cancellationOrder.items.reduce((sum, item) => sum + item.quantity, 0)} {cancellationOrder.items.reduce((sum, item) => sum + item.quantity, 0) === 1 ? "pieza" : "piezas"} al stock disponible.</p>
