...
export default function AccountArea() {
  ...
  if (checking) return <main className="account-loading"><a className="brand" href="/">PATA PAPAYA</a><p>Preparando tu espacio personal…</p></main>;

  if (!user) {
    return (
      <main className="account-auth">
        <header className="account-header"><a className="brand" href="/">PATA PAPAYA</a><a href="/">← Volver a la tienda</a></header>
        <section className="account-auth-layout">
          <div className="account-auth-copy"><p className="eyebrow">Tu rincón Pata Papaya</p><h1>Pedidos, favoritos<br /><em>y muchas carreras.</em></h1><p>Consulta pedidos, guarda los juguetes favoritos de tu mascota y reutiliza tus direcciones.</p><ul><li>Historial y cancelación de pedidos pendientes</li><li>Favoritos y direcciones guardadas</li><li>Sesión privada y contraseña cifrada</li></ul></div>
  ...
  return (
    <main className="account-page">
      <header className="account-header"><a className="brand" href="/">PATA PAPAYA</a><nav><a href="/">Tienda</a>{user.role === "admin" && <a href="/admin">Administración</a>}<button type="button" onClick={logout}>Cerrar sesión</button></nav></header>
      <section className="account-hero"><div><p className="eyebrow">Mi cuenta</p><h1>Hola, <em>{user.name.split(" ")[0]}.</em></h1><p>{user.email}</p></div><span>Parte de la manada · #{String(user.id).padStart(4, "0")}</span></section>
      ...
      {cancellationOrder && (
        <div className="account-confirmation-overlay" role="dialog" aria-modal="true" aria-labelledby="cancel-order-title">
          ...
          <section className="account-confirmation-panel">
            ...
            <p>Esta acción marcará el pedido como cancelado y devolverá sus {cancellationOrder.items.reduce((sum, item) => sum + item.quantity, 0)} {cancellationOrder.items.reduce((sum, item) => sum + item.quantity, 0) === 1 ? "juguete" : "juguetes"} al stock disponible.</p>
