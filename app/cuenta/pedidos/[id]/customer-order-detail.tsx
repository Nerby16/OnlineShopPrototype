...
export default function CustomerOrderDetail({ orderId }: { orderId: number }) {
  ...
  if (!order) {
    return <main className="order-detail-error"><a className="brand" href="/">LÚMINA</a><div><p className="eyebrow">No hemos podido abrirlo</p><h1>Este pedido no está<br /><em>disponible.</em></h1><p>{error}</p><a className="primary-link" href="/cuenta">Volver a mi cuenta <span>→</span></a></div></main>;
  }

  return (
    <main className="order-detail-page">
      <header className="account-header"><a className="brand" href="/">LÚMINA</a><nav><a href="/cuenta">← Mis pedidos</a><a href="/">Tienda</a></nav></header>
      ...
      <div className="order-detail-layout">
        <div className="order-detail-main">
          ...
          <section className="order-detail-section"><div className="order-detail-heading"><p className="eyebrow">Contenido</p><h2>{order.items.reduce((sum, item) => sum + item.quantity, 0)} piezas elegidas</h2></div><div className="order-detail-items">{order.items.map((item) => <article key={item.productId}><a href={`/productos/${item.productSlug}`}><img src={item.productImage} alt={item.productName} /></a><div><span>Producto</span><h3><a href={`/productos/${item.productSlug}`}>{item.productName}</a></h3><p>{item.quantity} × {money.format(item.unitPrice)}</p></div><strong>{money.format(item.unitPrice * item.quantity)}</strong></article>)}</div></section>
        </div>
