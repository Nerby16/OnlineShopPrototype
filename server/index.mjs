...
function validateProductImageUrl(value) {
  ...
  try {
    ...
    throw httpError(400, "La imagen debe usar HTTPS o proceder de las cargas locales de Nexo Animal.");
  }
}
...
async function ensureAdminUser() {
  ...
  await pool.execute(`
    ...
    VALUES (?, 'Nexo Animal Admin', ?, 'admin')
  ...
  try {
    ...
    if (nextStatus === "pending") {
      ...
      await connection.execute(`
        ...
        WHERE id = ?
      `, [`NEX-${String(orderId).padStart(6, "0")}`, orderId]);
    } else {
      await connection.execute(`
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/health") {
      ...
      sendJson(response, 200, { database: "connected", service: "nexo-animal-api" });
      return;
    }
...
async function start() {
  ...
  server.listen(port, "127.0.0.1", () => {
    console.log(`Nexo Animal API disponible en http://127.0.0.1:${port}`);
  });
}
...
start().catch((error) => {
  console.error("No se pudo iniciar Nexo Animal API:", error.message);
  process.exitCode = 1;
});
